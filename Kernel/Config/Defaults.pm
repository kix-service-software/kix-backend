...
#    $Self->{InfoFile} = 'AgentInfo';
    ...
    $Self->{ProxyCheckRemoteIP} = 1;

    # TokenMaxTime
    # (Max valid time of a token in seconds (8h = 28800).)
    $Self->{TokenMaxTime} = 16 * 60 * 60;

    # TokenMaxIdleTime
    # (After this time (in seconds) without new http request, then
    # the user get logged off)
    $Self->{TokenMaxIdleTime} = 6 * 60 * 60;
...
#        ReadOnly => 1,
        Map => [
            ...
            { Attribute => 'UserEmail',       Label => Translatable('Email'),               MappedTo => 'email',        Exposed => 1, Required => 1, DisplayGroup => 'Communication' },
            { Attribute => 'UserMobile',      Label => Translatable('Mobile'),              MappedTo => 'mobile',       Exposed => 1 },
            { Attribute => 'UserStreet',      Label => Translatable('Street'),              MappedTo => 'street',       Exposed => 1, DisplayGroup => 'Postal Address' },
            { Attribute => 'UserZip',         Label => Translatable('Zip'),                 MappedTo => 'zip',          Exposed => 1, DisplayGroup => 'Postal Address' },
            { Attribute => 'UserCity',        Label => Translatable('City'),                MappedTo => 'city',         Exposed => 1, DisplayGroup => 'Postal Address' },
...
#            #{ Attribute => 'UserCustomerIDs', Label => Translatable('CustomerIDs'),        MappedTo => 'second_customer_ids', Exposed => 1, Required => 1 },
...
#           { Attribute => 'UserAddress',      Label => Translatable('Address'),             MappedTo => 'postaladdress',       Exposed => 1, DisplayGroup => 'Postal Address' },
