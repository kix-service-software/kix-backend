...
sub Run {
    ...
    my $Success = $Kernel::OM->Get('Kernel::System::SLA')->SLAUpdate(
        ...
        UpdateTime              => $SLA->{UpdateTimeComment} || $SLAData{UpdateTime},
        UpdateNotify            => $SLA->{UpdateNotifyComment} || $SLAData{UpdateNotify},
        SolutionTime            => $SLA->{SolutionTimeComment} || $SLAData{SolutionTime},
        SolutionNotify          => $SLA->{SolutionNotifyComment} || $SLAData{SolutionNotify},
        MinTimeBetweenIncidents => $SLA->{MinTimeBetweenIncidentsComment} || $SLAData{MinTimeBetweenIncidents},
        UserID                  => $Self->{Authorization}->{UserID},               
    );
