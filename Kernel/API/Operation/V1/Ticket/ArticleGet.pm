...
sub Run {
    ...
    if ( $Self->_CanRunParallel(Items => $Param{Data}->{ArticleID}) ) {
        @ArticleList = $Self->_RunParallel(
            \&_GetArticleData,
            Items => $Param{Data}->{ArticleID},
            %Param,
        );
    }
    else {
        ...
        for my $ArticleID ( @{$Param{Data}->{ArticleID}} ) {
            my $ArticleData = $Self->_GetArticleData(
                ArticleID => $ArticleID,
                Data      => $Param{Data}
            );
...
sub _GetArticleData {
    ...
    my $ArticleID = $Param{Item} || $Param{ArticleID};
    ...
    my %ArticleRaw = $Kernel::OM->Get('Ticket')->ArticleGet(
        ArticleID     => $ArticleID,
    ...
    ATTRIBUTE:
    for my $Attribute ( keys %ArticleRaw ) {
    ...
    if ( $Param{Data}->{include}->{Attachments} || $Param{Data}->{include}->{'Article.Attachments'} ) {
        ...
        $Self->IncludeSubResourceIfProperty(
    ...
    if ( $Param{Data}->{include}->{Plain} ) {
        my $PlainMessage = $Kernel::OM->Get('Ticket')->ArticlePlain(
            ArticleID => $ArticleID,
            UserID    => $Self->{Authorization}->{UserID},
        ) || '';
