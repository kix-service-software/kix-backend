...
package Kernel::API::Validator::CustomerValidator;
...
Kernel::API::Validator::CustomerValidator - validator module
...
sub Validate {
    ...
    my $Found;
    if ( $Param{Attribute} eq 'CustomerContact' ) {
        $Found = $Kernel::OM->Get('Kernel::System::CustomerUser')->CustomerUserDataGet(
            User => $Param{Data}->{$Param{Attribute}},
        );
        if ( !$Found ) {
