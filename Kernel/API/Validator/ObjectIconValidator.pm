...
package Kernel::API::Validator::ObjectIconValidator;
...
Kernel::API::Validator::ObjectIconValidator - validator module
    ...
    if ( $Param{Attribute} eq 'Content' ) {
        ...
        if ( !$IsValid ) {
            ...
            return $Self->_Error(
                ...
                Message => "Validation of image failed! (error: $LogMessage)",
            );
        }
    }
    else {
        return $Self->_Error(
            ...
            Message => "ObjectIconValidator: cannot validate attribute $Param{Attribute}!",
        );
    }
