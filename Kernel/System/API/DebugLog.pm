...
package Kernel::System::API::DebugLog;
...
Kernel::System::API::DebugLog - log interface for generic interface
...
sub LogAdd {
    ...
    if (
        !$Kernel::OM->Get('Kernel::System::DB')->Do(
            SQL =>
                'INSERT INTO api_debugger_entry_content'
                . ' (content, create_time, debug_level, api_debugger_entry_id, subject)'
    ...
    if (
        !$DBObject->Prepare(
            SQL =>
                ...
                . ' webservice_id FROM api_debugger_entry WHERE communication_id = ?',
    ...
    if (
        !$DBObject->Prepare(
            SQL =>
                ...
                . ' FROM api_debugger_entry_content WHERE api_debugger_entry_id = ?'
...
sub LogDelete {
    ...
    my $SQLIndividual =
        'DELETE FROM api_debugger_entry_content
        WHERE api_debugger_entry_id in( SELECT id FROM api_debugger_entry ';
    my @BindIndividual;
    if ($CommunicationIDValid) {
    ...
    # delete main entry
    my $SQLMain = 'DELETE FROM api_debugger_entry WHERE';
    my @BindMain;
    if ($CommunicationIDValid) {
...
sub _LogAddChain {
    ...
    if (
        !$Kernel::OM->Get('Kernel::System::DB')->Do(
            SQL =>
                'INSERT INTO api_debugger_entry'
