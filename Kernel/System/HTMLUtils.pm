...
sub ToAscii {
    ...
    # forcing line breaks every 78 chars, if not disabled by parameter
    my $LineLength;
    if ( !$Param{NoForcedLinebreak} ) {
        $LineLength = 78;
    }
    ...
    # init url glossar, if not disabled by parameter
    ...
    if (
        defined( $Parser->{Link} )
        && IsArrayRefWithData( $Parser->{Link}->{List} )
    ) {
        $Param{String} .= "\n\n" . join( "\n", @{ $Parser->{Link}->{List} } );
    }

    # force line breaking, if not disabled
    if (
        $LineLength
...
sub _AsciiTextHandler {
    ...
    if ( !$Self->{Flag}->{Preformat} ) {
        ...
        # get currently trailing linebreaks
        my $AsciiEnd   = substr( $Self->{Ascii}, -1 );
        my $BreakCount = 0;
        if ( $AsciiEnd eq "\n" ) {
            $BreakCount = 1;
        }

        # remove leading whitespace on begin of line
        if ( $BreakCount ) {
            $Text =~ s/^\s*//mg;
        }
