...
sub UserSearch {
    ...
    if ( IsHashRefWithData($Param{HasPermission}) ) {
        ...
        my $ExcludedRoleConditionForSubSelects = '';
        if ( IsArrayRefWithData($ExcludeRoleIDs) ) {
            ...
            $ExcludedRoleConditionForSubSelects = 'AND (ru1.role_id NOT IN (' . join( ', ', @{ $ExcludeRoleIDs } ) . ')';
            ...
            if ( IsArrayRefWithData( $Param{ExcludeUsersByRoleIDsIgnoreUserIDs} ) ) {
                ...
                $ExcludedRoleConditionForSubSelects .= ' OR ru1.user_id IN ('. join( ', ', @{ $Param{ExcludeUsersByRoleIDsIgnoreUserIDs} } ) .')';
            }

            $ExcludedRoleCondition .= ')';
            push(@Where, $ExcludedRoleCondition);
            $ExcludedRoleConditionForSubSelects .= ')';
        }
        ...
        my @WherePart1 = (
            "EXISTS (SELECT rp1.id FROM roles r1, role_user ru1, role_permission rp1, permission_type pt1 WHERE r1.valid_id = 1 AND r1.id = ru1.role_id AND r1.usage_context IN (1,3) AND ru1.user_id = u.id AND ru1.role_id = rp1.role_id AND pt1.id = rp1.type_id AND pt1.name='Base::Ticket' $ExcludedRoleConditionForSubSelects)"
        );
        ...
        my @WherePart2 = (
            "NOT EXISTS (SELECT rp1.id FROM roles r1, role_user ru1, role_permission rp1, permission_type pt1 WHERE r1.valid_id = 1 AND r1.id = ru1.role_id AND r1.usage_context IN (1,3) AND ru1.user_id = u.id AND ru1.role_id = rp1.role_id AND pt1.id = rp1.type_id AND pt1.name='Base::Ticket' $ExcludedRoleConditionForSubSelects)"
        );
