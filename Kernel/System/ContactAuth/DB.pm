...
package Kernel::System::ContactAuth::DB;
...
sub new {
    ...
    $Self->{Table} = $ConfigObject->Get( 'Contact::AuthModule::DB::Table' . $Param{Count} )
        || die "Need Contact::AuthModule::DB::Table$Param{Count} in Kernel/Config.pm!";
    $Self->{ID} = $ConfigObject->Get( 'Contact::AuthModule::DB::Column::ID' . $Param{Count} )
        || die "Need Contact::AuthModule::DB::Column::ID$Param{Count} in Kernel/Config.pm!";
    $Self->{Login} = $ConfigObject->Get( 'Contact::AuthModule::DB::Column::Login' . $Param{Count} )
        || die "Need Contact::AuthModule::DB::Column::Login$Param{Count} in Kernel/Config.pm!";
    $Self->{Pw} = $ConfigObject->Get( 'Contact::AuthModule::DB::Column::Password' . $Param{Count} )
        || die "Need Contact::AuthModule::DB::Column::Password$Param{Count} in Kernel/Config.pm!";
    $Self->{CryptType} = $ConfigObject->Get( 'Contact::AuthModule::DB::CryptType' . $Param{Count} )
        || '';

    if ( $ConfigObject->Get( 'Contact::AuthModule::DB::DSN' . $Param{Count} ) ) {
        $Self->{DBObject} = Kernel::System::DB->new(
            DatabaseDSN =>
                $ConfigObject->Get( 'Contact::AuthModule::DB::DSN' . $Param{Count} ),
            DatabaseUser =>
                $ConfigObject->Get( 'Contact::AuthModule::DB::User' . $Param{Count} ),
            DatabasePw =>
                $ConfigObject->Get( 'Contact::AuthModule::DB::Password' . $Param{Count} ),
            Type => $ConfigObject->Get( 'Contact::AuthModule::DB::Type' . $Param{Count} )
                || '',
            )
...
sub Auth {
    ...
    $Self->{DBObject}->Prepare(
        SQL => "
            SELECT $Self->{Pw}, $Self->{ID} FROM $Self->{Table} WHERE
            $Self->{Login} = ?
            ",
        Bind => [ \$Param{User} ],
    );
