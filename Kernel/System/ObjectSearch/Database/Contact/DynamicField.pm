...
package Kernel::System::ObjectSearch::Database::Contact::DynamicField;
...
Kernel::System::ObjectSearch::Database::Contact::DynamicField - attribute module for database object search
...
sub Search {
    ...
    if ( $Param{BoolOperator} eq 'OR') {
        push(
            @SQLJoin,
            <<"END"
LEFT JOIN dynamic_field_value $JoinTable ON c.id = $JoinTable.object_id
...
END
        ...
        push(
            @SQLJoin,
            <<"END"
INNER JOIN dynamic_field_value $JoinTable ON c.id = $JoinTable.object_id
    AND $JoinTable.field_id = $DynamicFieldConfig->{ID}
END
        );
    }
...
sub Sort {
    ...
    if ( !$JoinTable ) {
        ...
        push(
            @SQLJoin,
            <<"END"
LEFT OUTER JOIN dynamic_field_value $JoinTable ON c.id = $JoinTable.object_id
AND $JoinTable.field_id = $DynamicFieldConfig->{ID}
...
END
    ...
    return {
        Join    => \@SQLJoin,
        Select  => [$SQLOrderField],
        OrderBy => [$SQLOrderField]
    };
}
