...
sub Search {
    ...
    if ( !defined( $Param{Flags}->{JoinMap}->{ 'DynamicField_' . $DFName } ) ) {
        ...
        $TableAlias .= $Count;
        push( @SQLJoin, "LEFT OUTER JOIN dynamic_field_value $TableAlias ON $TableAlias.object_id = st.id AND $TableAlias.field_id = $DynamicFieldConfig->{ID}" );
...
sub Sort {
    ...
    if ( !defined( $Param{Flags}->{JoinMap}->{ 'SortDynamicField_' . $DFName } ) ) {
        my $Count = $Param{Flags}->{DynamicFieldJoinCounter}++;
        $TableAlias .= $Count;
        push( @SQLJoin, "LEFT OUTER JOIN dynamic_field_value $TableAlias ON $TableAlias.object_id = st.id AND $TableAlias.field_id = $DynamicFieldConfig->{ID} AND $TableAlias.first_value = 1" );
