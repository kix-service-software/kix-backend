# --
# Copyright (C) 2006-2025 KIX Service Software GmbH, https://www.kixdesk.com/
# --
# This software comes with ABSOLUTELY NO WARRANTY. For details, see
# the enclosed file LICENSE-GPL3 for license information (GPL3). If you
# did not receive this file, see https://www.gnu.org/licenses/gpl-3.0.txt.
# --

package Kernel::System::ObjectSearch::Database::FAQArticle::Category;

use strict;
use warnings;

use Kernel::System::VariableCheck qw(:all);

use base qw(
    Kernel::System::ObjectSearch::Database::CommonAttribute
);

our $ObjectManagerDisabled = 1;

=head1 NAME

Kernel::System::ObjectSearch::Database::FAQArticle::Category - attribute module for database object search

=head1 SYNOPSIS

=head1 PUBLIC INTERFACE

=over 4

=cut

sub GetSupportedAttributes {
    my ( $Self, %Param ) = @_;

    return {
        Category => {
            IsSearchable => 1,
            IsSortable   => 1,
            Operators    => ['EQ','NE','STARTSWITH','ENDSWITH','CONTAINS','LIKE','IN','!IN']
        },
        CategoryID => {
            IsSearchable => 1,
            IsSortable   => 1,
            Operators    => ['EQ','NE','IN','!IN'],
            ValueType    => 'NUMERIC'
        }
    };
}

sub Search {
    my ( $Self, %Param ) = @_;
    my @SQLJoin;

    # check params
    return if !$Self->_CheckSearchParams(%Param);

    my $ValueType  = q{};
    my $TableAlias = $Param{Flags}->{FlagMap}->{CategoryJoin} // 'fc';
    if (
        !$Param{Flags}->{FlagMap}->{CategoryJoin}
        && $Param{Search}->{Field} eq 'Category'
    ) {
        my $Count = $Param{Flags}->{CategoryJoinCounter}++;
        $TableAlias .= $Count;
        push (
            @SQLJoin,
            "INNER JOIN faq_category $TableAlias ON $TableAlias.id = f.category_id"
        );
        $Param{Flags}->{CategoryJoin} = $TableAlias;
    }

    my %AttributeMapping = (
        Category   => {
            Column => "$TableAlias.name",
        },
        CategoryID => {
            Column    => 'f.category_id',
            ValueType => 'NUMERIC'
        }
    );

    my $Condition = $Self->_GetCondition(
        Operator  => $Param{Search}->{Operator},
        Column    => $AttributeMapping{$Param{Search}->{Field}}->{Column},
        Value     => $Param{Search}->{Value},
        ValueType => $AttributeMapping{$Param{Search}->{Field}}->{ValueType}
    );

    return if ( !$Condition );

    return {
        Join  => \@SQLJoin,
        Where => [ $Condition ]
    };
}

sub Sort {
    my ( $Self, %Param ) = @_;

    # check params
    return if !$Self->_CheckSortParams(%Param);

    my @SQLJoin;
    my $TableAlias = $Param{Flags}->{FlagMap}->{CategoryJoin} // 'fc';
    if (
        !$Param{Flags}->{FlagMap}->{CategoryJoin}
        && $Param{Attribute} eq 'Category'
    ) {
        my $Count = $Param{Flags}->{CategoryJoinCounter}++;
        $TableAlias .= $Count;
        push (
            @SQLJoin,
            "INNER JOIN faq_category $TableAlias ON $TableAlias.id = f.category_id"
        );
        $Param{Flags}->{FlagMap}->{CategoryJoin} = $TableAlias;
    }

    my %AttributeMapping = (
        Category   => "$TableAlias.name",
        CategoryID => 'f.category_id'
    );

    return {
        Select  => [$AttributeMapping{$Param{Attribute}}],
        OrderBy => [$AttributeMapping{$Param{Attribute}}],
        Join    => \@SQLJoin
    };
}

1;


=back

=head1 TERMS AND CONDITIONS

This software is part of the KIX project
(L<https://www.kixdesk.com/>).

This software comes with ABSOLUTELY NO WARRANTY. For details, see the enclosed file
LICENSE-GPL3 for license information (GPL3). If you did not receive this file, see

<https://www.gnu.org/licenses/gpl-3.0.txt>.

=cut
