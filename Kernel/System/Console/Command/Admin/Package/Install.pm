...
sub Configure {
    ...
    $Self->Description('Install a KIX package.');
    $Self->AddOption(
        Name        => 'force',
    ...
    $Self->AddArgument(
        Name => 'location',
        Description =>
            "Specify a file path, a remote repository (http://packages.kixdesk.com/plugins/:Package-1.0.0.kpm) or just any online repository (online:Package).",
        Required   => 1,
        ValueRegex => qr/.*/smx,
    );
...
sub Run {
    ...
    # install
    my $Success = $Kernel::OM->Get('Package')->PackageInstall(
        String => $FileString,
