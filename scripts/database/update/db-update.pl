...
if (!%Opts) {
    print "USAGE: db-update.pl -s <Start> -t <End>\n";
    exit 0;
}
...
if (!$StartVersion) {
    print STDERR "Wrong start version format ($Opts{s})!\n"; 
    exit 1;
}
...
if (!$TargetVersion) {
    print STDERR "Wrong end version format ($Opts{t})!\n"; 
    exit 1;
}

# get all relevant versions between Start (s) and to (t)
my @FileList = $Kernel::OM->Get('Kernel::System::Main')->DirectoryRead(
    ...
    Filter    => 'db-update-build-*',
);

my %VersionList;
foreach my $File (sort @FileList) {
    my ($Filename, $Dirs, $Suffix) = fileparse($File, qr/\.[^.]*/);
    if ($Filename =~ /^db-update-build-(\d+).*?$/g) {
        my $NummericVersion = "$1";
        $VersionList{$NummericVersion} = "build-$1";
    }
}
