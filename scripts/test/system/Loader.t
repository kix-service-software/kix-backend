...

{
    my $CSS = $MainObject->FileRead(
        Location => $Home . '/scripts/test/system/sample/Loader/OTRS.Reset.css',
    );

    $CSS = ${$CSS};

    my $ExpectedCSS = $MainObject->FileRead(
        Location => $Home . '/scripts/test/system/sample/Loader/OTRS.Reset.min.css',
    );
...

{
    my $JavaScript = $MainObject->FileRead(
        Location => $Home . '/scripts/test/system/sample/Loader/OTRS.Agent.App.Login.js',
    );
    ...
    my $ExpectedJS = $MainObject->FileRead(
        Location => $Home . '/scripts/test/system/sample/Loader/OTRS.Agent.App.Login.min.js',
    );
...

{
    my $MinifiedJSFilename = $LoaderObject->MinifyFiles(
        List => [
            $Home . '/scripts/test/system/sample/Loader/OTRS.Agent.App.Login.js',
            $Home . '/scripts/test/system/sample/Loader/OTRS.Agent.App.Dashboard.js',
        ],
        Type            => 'JavaScript',
    ...
    my $Expected = $MainObject->FileRead(
        Location => $Home . '/scripts/test/system/sample/Loader/CombinedJavaScript.min.js',
    );
    $Expected = ${$Expected};
    $Expected =~ s{\r\n}{\n}xmsg;
