...
for my $Test (@Tests) {
    ...
    if ( $Test->{SuccessCreate} ) {
        ...
        if ( $Test->{ExternalCustomer} ) {
            $Self->Is(
                $LocalTicketData{ContactID},
                undef,
                "$Test->{Name} - local Ticket->Contact is empty.",
            );
        }
        else {
            $Self->Is(
                $LocalTicketData{ContactID},
                $Test->{RequestData}->{Ticket}->{Contact},
                "$Test->{Name} - local Ticket->Contact match test definition.",
            );
        }
        ...
        for my $Attribute (qw(Channel SenderType)) {
