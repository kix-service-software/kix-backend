...
for my $Count ( 1 .. 3 ) {
    ...
    my $CertString = $MainObject->FileRead(
        Directory => $ConfigObject->Get('Home') . "/scripts/test/system/sample/SMIME/",
...
',
    ...
    for my $Number ( 0 .. 3 ) {
        ...
        $CertInfo{ 'SmimeTest_' . $Number }->{Subject} =
            'C= MX ST= Jalisco L= Guadalajara O= KIX OU= Research and Development CN= test.org emailAddress= smime@test.com';
